...
execute if score #menuHidden var = #TRUE var run playsound minecraft:ui.button.click master @a[x=500,y=50,z=0,distance=0..1,tag=menuRequest,tag=menuAdmin,tag=!menuTimeout] ~ ~ ~ 1 0.8 1
execute if score #menuHidden var = #FALSE var unless score #gameState var = #IDLE var run playsound minecraft:ui.button.click master @a[x=500,y=50,z=0,distance=0..1,tag=menuRequest,tag=menuAdmin,tag=!menuTimeout] ~ ~ ~ 1 0.8 1
